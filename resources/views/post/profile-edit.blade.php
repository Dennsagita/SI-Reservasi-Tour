            ...
            <section class="bg-white">
                    ...
                    <form action="{{ route('profileupdate') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" name="nama" id="nama" value="{{ Auth::guard('user')->user()->nama }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Masukan nama anda" required="">
                        </div>
                        <div class="w-full">
                            <label for="no_telp" class="block mb-2 text-sm font-medium text-gray-900 ">Nomor Telphone</label>
                            <input type="text" name="no_telp" id="no_telp" value="{{ Auth::guard('user')->user()->no_telp }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Contoh: 081234555666" required="">
                        </div>
                        <div class="w-full">
                            <label for="alamat" class="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                            <input type="text" name="email" id="email" readonly value="{{ Auth::guard('user')->user()->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Masukan alamat" required="">
                        </div>
                        <div class="w-full">
