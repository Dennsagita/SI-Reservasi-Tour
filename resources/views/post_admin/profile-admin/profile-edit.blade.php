...
@section('action')
@section('title', 'Profile Edit Admin')
    ...
    <div class="relative flex flex-col flex-auto min-w-0 p-4 mx-6 -mt-16 overflow-hidden break-words border-0 shadow-blur rounded-2xl bg-white/80 bg-clip-border backdrop-blur-2xl backdrop-saturate-200">
        ...
        <div class="flex-none w-auto max-w-full px-3">
            <div class="text-base ease-soft-in-out h-18.5 w-18.5 relative inline-flex items-center justify-center rounded-xl text-white transition-all duration-200">
                <img src="{{  asset($admin->images->count() ? 'storage/' . $admin->images->first()->src : 'assets/images/blog/user-1.png') }}" alt="" class="w-full shadow-soft-sm rounded-xl" />
            </div>
        </div>
            ...
            <section class="bg-white">
                    ...
                    <form action="{{ route('profileUpdateAdmin') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="w-full">
                            ...
                            <input type="text" name="nama" id="nama" value="{{ Auth::guard('admin')->user()->nama }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Masukan nama anda" required="">
                        </div>
                        <div class="w-full">
                            <label for="no_telp" class="block mb-2 text-sm font-medium text-gray-900 ">Nomor Telphone</label>
                            <input type="text" name="no_telp" id="no_telp" value="{{ Auth::guard('admin')->user()->no_telp }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Contoh: 081234555666" required="">
                        </div>
                        <div class="w-full">
                            <label for="alamat" class="block mb-2 text-sm font-medium text-gray-900 ">Email</label>
                            <input type="text" name="email" id="email" readonly value="{{ Auth::guard('admin')->user()->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Masukan alamat" required="">
                        </div>
                        <div class="w-full">
                            <label for="alamat" class="block mb-2 text-sm font-medium text-gray-900 ">Alamat</label>
                            <input type="text" name="alamat" id="alamat" value="{{ Auth::guard('admin')->user()->alamat }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Masukan alamat" required="">
                        </div>
                        <div class="upload__box">
                            ...
                            <div class="upload__img-wrap">
                                @foreach ($admin->images as $item => $image)
